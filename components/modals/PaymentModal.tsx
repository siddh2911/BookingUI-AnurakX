    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Add Payment">
      <form onSubmit={handleAddPayment} className="space-y-4">
              ...
              <tbody className="divide-y divide-slate-100">
                {payments.map(p => (
                  <tr key={p.id}>
                    ...
                    <td className="px-2 py-1">{p.bookingSource || 'Unknown'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Allocated Source</label>
            <select name="bookingSource" className="w-full border rounded-lg p-2 text-sm">
              <option value="">Auto / Primary ({selectedBooking.sources?.[0]?.source})</option>
              {selectedBooking.sources?.map((s, idx) => (
                <option key={idx} value={s.source}>{s.source}</option>
              ))}
            </select>
          </div>
        </div>
