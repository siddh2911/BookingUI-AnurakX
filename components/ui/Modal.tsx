...
const Modal = ({ isOpen, onClose, title, children, maxWidth = "max-w-4xl" }: { isOpen: boolean; onClose: () => void; title: string; children?: React.ReactNode; maxWidth?: string }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-2 md:p-4 backdrop-blur-md">
      <div className={`bg-white/90 backdrop-blur-2xl rounded-xl shadow-2xl w-full ${maxWidth} max-h-[95vh] flex flex-col border border-white/20 transition-colors duration-1000`}>
        <div className="flex justify-between items-center p-3 md:p-6 bg-white/5 border-b border-white/10 shrink-0">
        ...
        <div className="p-3 md:p-8 bg-slate-50/10 overflow-y-auto flex-1">{children}</div>
      </div>
    </div>
  );
};
