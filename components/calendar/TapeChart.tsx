...
export default function TapeChart({ rooms, bookings, onBookingClick }: TapeChartProps) {
    ...
    return (
        <div className="bg-white/70 backdrop-blur-xl transition-colors duration-1000 rounded-2xl shadow-sm border border-white/20 flex flex-col h-[calc(100vh-140px)]">
            { }
            { }
            <div className="p-4 border-b border-white/10 flex flex-col md:flex-row items-center justify-between gap-4 bg-white/5 relative z-10 shadow-[0_4px_24px_-12px_rgba(0,0,0,0.1)]">
                <div className="flex items-center justify-between w-full md:w-auto gap-4">
                    <div className="flex items-center gap-1 bg-white/20 backdrop-blur-md p-1 rounded-lg border border-white/30">
                        <button onClick={handlePrev} className="p-1.5 hover:bg-white/50 rounded-md transition duration-200 hover:shadow-sm text-slate-700">
                            <ChevronLeft size={20} />
                        </button>
            ...
            { }
            <div className="flex-1 overflow-auto relative custom-scrollbar bg-slate-50/20">
                <div className="min-w-[800px] md:min-w-[1200px]">
                    { }
                    <div className="flex sticky top-0 z-20 bg-white/40 backdrop-blur-md shadow-sm border-b border-white/20">
                        <div className="w-20 md:w-48 flex-shrink-0 p-2 md:p-4 font-bold text-slate-700 border-r border-white/20 bg-white/30 backdrop-blur-md flex items-center justify-center md:justify-start sticky left-0 z-30 shadow-[4px_0_24px_-12px_rgba(0,0,0,0.1)]">
                        ...
                        <div className="flex-1 flex">
                            {dates.map(date => (
                                <div
                                    ...
                                    className={`flex-shrink-0 p-2 text-center border-r border-white/10 ${isToday(date) ? 'bg-blue-500/10' : ''}`}
                    ...
                    { }
                    {rooms.map(room => (
                        <div key={room.id} className="flex border-b border-white/10 bg-white/5 hover:bg-white/10 transition-colors h-20 group">
                            <div className="w-20 md:w-48 flex-shrink-0 p-2 md:p-3 border-r border-white/10 flex flex-col justify-center sticky left-0 z-10 bg-white/5 group-hover:bg-transparent backdrop-blur-md text-center md:text-left shadow-[4px_0_24px_-12px_rgba(0,0,0,0.1)]">
                                <div className="flex flex-col md:flex-row items-center md:justify-between">
                                    <span className="font-bold text-slate-800 text-sm md:text-lg">{room.number}</span>
                                    <span className={`text-[8px] md:text-[10px] px-1 md:px-1.5 py-0.5 rounded font-bold uppercase hidden md:inline-block ${room.status === RoomStatus.AVAILABLE ? 'bg-green-500/10 text-emerald-600' : 'bg-slate-500/10 text-slate-500'}`}>
                                        {room.status}
                                    </span>
                                </div>
                            ...
                            <div className="flex-1 relative">
                                ...
                                <div className="absolute inset-0 flex pointer-events-none">
                                    {dates.map(date => (
                                        <div
                                            key={date.toISOString()}
                                            className={`flex-shrink-0 border-r border-white/5 ${isToday(date) ? 'bg-blue-500/10' : ''}`}
                                            style={{ width: `${100 / daysToShow}%` }}
                                        ></div>
                                    ))}
                                </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="truncate opacity-80 text-[10px]">
                                                {booking.sources?.[0]?.source === 'Booking.com' ? 'Booking.com' : booking.sources?.[0]?.source === 'Airbnb' ? 'Airbnb' : 'Direct'}
                                            </div>
                                        </div>
                                    );
                                })}
